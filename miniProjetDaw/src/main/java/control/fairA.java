...
public class fairA extends HttpServlet {
		...
		if (request.getSession().getAttribute("user") != null){
			...
			try {
				//etudListG = userDAOEtud.getEtudiant();
					...
					RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/faireApl.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			System.out.printf("this is the group",group);
				...
				request.setAttribute("etudListG",etudListG);
				 dispatcher1 = request.getRequestDispatcher("/WEB-INF/faireApl.jsp");
